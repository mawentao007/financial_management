(function() {
  ...
  UserService = (function() {
    ...
    UserService.prototype.updateUser = function(userName, email, user) {
      var deferred;
      this.$log.debug("updateUser " + (angular.toJson(user, true)));
      deferred = this.$q.defer();
      this.$http.put("/user/" + userName + "/" + email, user).success((function(_this) {
        return function(data, status, headers) {
    ...
    UserService.prototype.deleteUser = function(userName, email) {
      var deferred;
      deferred = this.$q.defer();
      this.$http["delete"]("/users/" + userName + "/" + email).success((function(_this) {
        return function(data, status, headers) {
          _this.$log.info("Successfully updated User - status " + status);
          return deferred.resolve(data);
        };
